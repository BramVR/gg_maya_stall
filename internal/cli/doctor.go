...
func runDoctor(repoDir string, options doctorOptions) doctorReport {
	...
	if options.ScenarioName != "" {
		...
		if !ok {
			add(failedCheck("scenario-inputs", fmt.Sprintf("unknown Scenario %q", options.ScenarioName), "Choose a configured Scenario or add it to the repo config."))
		} else {
			scenario = selected
			if err := validateScenarioInputs(repoDir, options.ScenarioName, scenario); err != nil {
				add(failedCheck("scenario-inputs", err.Error(), "Fix the Scenario payload paths and expectedOutputs.scenarioResult in repo config."))
			} else {
	...
	if err != nil {
		add(failedCheck("target-profile", err.Error(), "Point --host-config at a valid user host config or omit it for the fake default."))
		return report
	}
	profile, ok := hostConfig.TargetProfiles[options.TargetProfile]
	if !ok {
		add(failedCheck("target-profile", fmt.Sprintf("unknown Target Profile %q", options.TargetProfile), "Choose a configured Target Profile or add it to the host config."))
		return report
	}
	...
	if !ok {
		add(failedCheck("host-pool", fmt.Sprintf("unknown Host Pool %q", profile.HostPool), "Fix the Target Profile hostPool reference in the host config."))
		return report
	}
	if len(pool.Hosts) == 0 {
		add(failedCheck("host-pool", fmt.Sprintf("%s has no Maya Hosts", profile.HostPool), "Add at least one Maya Host to the Host Pool."))
		return report
	}
	for _, host := range pool.Hosts {
		if err := validateHostID(host.ID); err != nil {
			add(failedCheck("host-pool", err.Error(), "Fix Maya Host ids in the Host Pool config."))
			return report
		}
	}
	if !hostPoolHasHealthyHost(pool.Hosts) {
		add(failedCheck("host-pool", fmt.Sprintf("%s has no healthy Maya Hosts", profile.HostPool), "Mark a ready Maya Host healthy or repair one host before running."))
		return report
	}
	...
	if err != nil {
		add(failedCheck("host", err.Error(), "Choose a Maya Host from the selected Target Profile or repair Host Pool config."))
		return report
	}
...
func checkHostLayers(repoDir string, options doctorOptions, host mayaHostConfig, scenario scenarioConfig, add func(doctorCheck)) {
	add(statusLayer("fake-ssh", host.SSH, "reachable", []string{"", "ok", "healthy", "reachable"}, "Fix SSH reachability for this Maya Host."))
	add(statusLayer("work-root", host.WorkRoot, "writable", []string{"", "ok", "writable"}, "Fix the host work root path or permissions."))
	add(statusLayer("session-broker", host.Broker, "reachable", []string{"", "ok", "healthy", "reachable"}, "Start or repair the Session Broker on this Maya Host."))
	add(mayaVersionLayer(options, host, scenario))
	if host.VisualEvidence != nil && !*host.VisualEvidence {
		add(failedCheck("visual-evidence", "unavailable", "Enable screenshot or recording capture through the Session Broker."))
	} else {
...
func mayaVersionLayer(options doctorOptions, host mayaHostConfig, scenario scenarioConfig) doctorCheck {
	...
	return failedCheck("maya-version", fmt.Sprintf("Scenario %s needs %s; host has %s", options.ScenarioName, scenario.MayaVersion, installed), "Install a compatible Autodesk Maya version or choose another Maya Host.")
}
...
func hostLockLayer(repoDir string, hostID string) doctorCheck {
	...
	if err != nil {
		return failedCheck("host-lock", err.Error(), "Inspect the Host Lock state directory and permissions.")
	}
	stale, err := isStaleHostLock(lockPath)
	if err != nil {
		return failedCheck("host-lock", err.Error(), "Inspect or remove the unreadable Host Lock file.")
	}
	if stale {
		return okCheck("host-lock", "unlocked")
	}
	return failedCheck("host-lock", fmt.Sprintf("%s locked", hostID), "Wait for the active Fresh Run or clear the stale Host Lock after verifying no run is active.")
}
