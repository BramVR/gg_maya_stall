...
- Adapted in code: normal recording defaults from `internal/cli/artifacts.go`, currently `10s` at `15fps`.
...
- Adapted: artifact URL and Review Comment markdown patterns from `internal/cli/artifacts_publish.go`, rewritten around Evidence Bundles and Review Comments.
- Adapted: safe artifact/path handling patterns from `internal/cli/run_artifacts.go`, rewritten around Scenario Run Payloads and Evidence Bundles.
